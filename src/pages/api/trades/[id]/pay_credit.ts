...
// Adds credit to internal ledger and updates Shopify customer metafield with the new balance
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Compute new balance (sum deltas)
  const { data: rows, error: errSum } = await supabase.from("store_credit_ledger")
    .select("delta_cents")
    .eq("shopify_customer_id", trade.shopify_customer_id);
  if (errSum) return res.status(500).json({ error: errSum.message });
  const balance = (rows || []).reduce((a, r) => a + (r.delta_cents || 0), 0);

  // Update metafield on Shopify
  try {
    await setCustomerCreditCents(trade.shopify_customer_id, balance);
  } catch (e: any) {
    return res.status(500).json({ error: "Shopify metafield update failed: " + e.message });
  }

  // Mark trade paid
  const { data: updated, error: errUpd } = await supabase.from("trades")
